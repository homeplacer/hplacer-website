...
portal/
  ...
  src/
    ...
    integrations/         Monday sync and guarded Gmail poller, vendor-mail
                          parsing, private attachment staging, and review queue
...
| `GMAIL_CLIENT_ID`, `GMAIL_CLIENT_SECRET`, `GMAIL_REFRESH_TOKEN` | Optional vendor-mail ingestion | Worker secrets only, after confirming Workspace/project ownership, internal or external audience, OAuth audience, administrator approval/allowlisting, and retention policy. The importer stays disabled unless `GMAIL_INGEST_ENABLED=true` and `GMAIL_ALLOWED_MAILBOX=brandon@hplacer.com`. It uses Gmail read-only scope, stores no message bodies, stages allowlisted files privately, and requires employee review before linking. See `docs/operations/gmail-parts-email-integration-plan.md` |
...
Without optional integrations, the portal runs with the two narrowly scoped
application secrets for warranty and Careers intake, plus the deploy-time
Cloudflare token. Gmail stays disabled until its organization/audience path,
any required administrator approval, retention policy, and explicit secret
provisioning are confirmed. The Monday token is operator
tooling and is never needed by the deployed Worker.

### 6. Optional, once it is live

- Keep the daily `0 11 * * *` trigger (07:00 Eastern in summer) for low-stock,
  service-due, and insurance-expiration alerts. The separate `*/15 * * * *`
  trigger runs the Gmail importer only when it is explicitly enabled and all
  mailbox settings are present. The daily sweep can also be run from `/admin`.
