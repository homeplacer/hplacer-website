...
Brandon authorized mailbox access for this work. This implementation uses only `https://www.googleapis.com/auth/gmail.readonly`; it does not send, label, modify, or delete mail. Google classifies this scope as restricted. Google's current guidance says storing or transmitting restricted-scope data on a server requires a security assessment. An Internal Workspace OAuth audience may avoid some OAuth verification steps, but do not assume that removes the restricted-data security assessment. Before connecting Brandon's mailbox, the Workspace administrator must confirm the app audience, applicable Google verification/security-assessment obligations, and the company's retention policy.
...
1. **External policy/account setup (blocked):** confirm Workspace ownership, OAuth audience, Google restricted-scope security-assessment requirements, and retention policy. Create a Google OAuth client with only `gmail.readonly` and offline access. Verify the OAuth grant's Gmail profile is exactly `brandon@hplacer.com`.
...
Gmail implementation references: [Gmail OAuth scopes](https://developers.google.com/workspace/gmail/api/auth/scopes), [messages.list query and pagination](https://developers.google.com/workspace/gmail/api/reference/rest/v1/users.messages/list), [push watch and expiry limitations](https://developers.google.com/workspace/gmail/api/guides/push), and [Google Workspace API data policy](https://developers.google.com/workspace/workspace-api-user-data-developer-policy).
