...
3. Review this implementation. Apply migration `0016_vendor_email_parts_tracking.sql` and deploy only after an explicit user instruction, per `AGENTS.md`.
...
## Review fixes after employee portal release

The integration is reconciled locally with the released employee portal in an isolated readiness worktree. Its unapplied migration is now `0016_vendor_email_parts_tracking.sql`; `0014_equipment_parts_library.sql` and `0015_repair_work_details.sql` already belong to the portal release. Do not apply the obsolete Gmail migration 0014 from the older PR branch.

- Polling takes an atomic, expiring one-hour lease; concurrent invocations skip, and failures release their own lease. A terminated run can recover after lease expiry. Gmail/token requests time out after 30 seconds.
- A paginated scan persists its original query start and initial poll timestamp. Completion advances only to that original timestamp, so mail arriving during a long scan is picked up on the next scan.
- Numeric tracking requires a tracking label; phone/invoice numbers and ambiguous multiple-package matches are left unlinked. Canonical tracking validation must match the entire value. Unlabeled order prose is not accepted as an order ID. This conservative parser is not an exhaustive carrier-format validator.
- Review uses a transactional target-status check. Receiving/cancelling a request during review cannot reopen it; the reviewer must reload. Notifications commit atomically with the link, preventing lost alerts after a partial success.
- Poll failures store only a generic error code, never provider responses or tokens. Review queue must surface failed imports separately from an empty queue. Partial message failures are retried on later polls.

Google sign-in was verified, but the Cloud project picker exposed Forturro as the available organization. Whether Home Placer shares that Workspace still requires administrator verification. No OAuth client, mailbox access, production secrets, Gmail migrations or Gmail deployment were performed in this review. The separate Codex Gmail connector, if connected, does not provision credentials for the portal Worker.

The importer records selected sender/subject headers, extracted fields and private attachments; it does not store message bodies. It currently reads plain-text MIME parts and Gmail snippets, so HTML-only messages or unsupported tracking formats may require manual handling. Retention/deletion policy and administrator ownership must be resolved before enabling production ingestion. In-portal notifications are implemented; email/SMS delivery and a vendor-reply monitor are separate capabilities and are not enabled by this change.
